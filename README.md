...
- **GID** — уникальный текстовый идентификатор канала (произвольный, используется внутри системы)
- **Telegram ID** — числовой ID чата в Telegram
- **Имя** — отображаемое имя канала
- **Username** — @username канала (опционально)
- **Бот** — выбор из зарегистрированных ботов
...
**Поведение:**
- Training Mode — пассивный режим, спам детектируется, но не банится
- Dry Mode — сухой запуск, логирование без действий
- Soft Ban — ограничение действий вместо полного бана
- No Spam Reply — не отвечать на спам-сообщения
- Aggressive Cleanup — удалять все сообщения пользователя при бане
- Cleanup Limit — максимум сообщений для удаления
- Suppress Join Message — удалять сообщение о входе при бане
- Delete Join Messages — удалять все сообщения о входе
- Delete Leave Messages — удалять все сообщения о выходе
...
Действия: добавить сообщение в спам-сэмплы для дообучения классификатора.
...
Белый список пользователей, сообщения которых не проверяются на спам. Управление по user ID.

#### Сэмплы

Управление обучающими данными классификатора Байеса. Два типа:
- **Спам-сэмплы** — примеры спам-сообщений
- **Хам-сэмплы** — примеры легитимных сообщений

Сэмплы общие для всех каналов. Добавление, удаление, перезагрузка классификатора.

#### Словарь

Управление стоп-фразами и игнорируемыми словами:
- **Стоп-фразы** — слова/фразы, при наличии которых сообщение считается спамом
- **Игнорируемые слова** — исключения из проверки

Словарь общий для всех каналов.
...
| Одобренные пользователи | Общие для всех каналов |
